...
// us states 
M=`èèhëq~,<0G!@1<XAI/+MAKBWQLRZIbzraT[8Qi0		zqzML
...
// places
O=`Hac™”Nó]Ç?àGÇKÄ9LM”Nó]Ç?íQäKõ1Ñ;ÇEÄ?àF??1UÉG}TLM-XMMÇ?KQàF/S~NMLäKàGNùßfäK+?òd*Rßf*RîPÑ;¸¿íõ1ï°‚W∑\`äKòh…Ö3FIH∑\`ùké;Ä:8FKQéªÎªÇKãMÑ;4±GdCJâECJä1Ä9ïKMüéªÆX w¥Xà?µ[ö\\”ñBlíDïL›≠ä8ìS6O·Vö]rê8Åä`.match(/.{2}/g),

// lakes
P=`ECQ0K~ÖD~CFbpI~P¢8~óäa	)~(
QxI~ôòqi1~3$R8Y~Dyp~}/~÷t~ : paXQ~KGR
~÷s~EDxi@~óó9)~FLX~<:9iqX~ñ'yP~µF(
XI~ô"iy(8(~)I~Œe~.:~+R~,=`.split("~"),

// streams 
Q=`_åQ@Q@TBRAbqiypMèYAiqWè[pQkzY9Zé[qai\\çhaxiapaSè0JAH9~ó\\iXziZqqYhqaHhqiòtxhr ~âí)	zjpbhåéhq~ÖÑ!0@(;1HZQaPÖÑ!0	p~B	php~ö¶1@JYH9(0 ~±h(0(! ~A_9!(9"9A( ~0\\0 0~(By	()1y~Wº"	y~◊\\@HZ@Q0IYIiH8@1(:HP~’RQaJ0 ( APbXIXpxI8@~—j8J0HQ(~•B!1
...
// deserts 
R=`ws3wWOOO~0X)zjQ~öav[U3~…N#[Ta~‡πDR:b~ﬂØ"c~M°t\\*!1 ~†\\;Zk~ﬂ∞JG
z~å≥2 4{kr~4Zui;B:3~ç™jcB~Qﬂ9)1Elcs~™R:|R~2_0Iq;*|i~¨MrM~∂NjT~µi;j	~⁄Æ[:~´d!:K~≠ds~0WB)~ßViI~™_2
r`.split("~"),

// volcanoes
S=`ãRãL®®Vus!*BmãOAt÷s˘•◊v˜ œîùwAuˆˇ›ZïãÏënKÜÀë‘ñLÄ`.match(/.{2}/g),

// mountains
T=`Ω\`∂Tº\`„UõéÖD;oêN∏W˘Ãæaæaæaª\`º\`∑V∏V∏WΩaøb"pN”œWüHÜD,R2FQ©âC~0+@”Å”^∫\\/7`.match(/.{2}/g),
...
// seas
V=`D/ 3?}F#I)L$AM7o$o:m/i;T#R&L.wP\r$:+.G>=C$D'd?gImNADR2W<N:m\`A
...
var datasets = [
  ...
  [
    "SEA",                  // dataset name
    K,                      // names
    V,                      // center and radius for each sea (2b + 1b)
  ...
  [
    "DESERT",               // dataset name
    G,                      // names
    R,                      // polygons for each country (array of strings, 2b + 1b per point)
  ...
  [
    "FOREST",               // dataset name
    J,                      // names
    U,                      // polygons for each country (array of strings, 2b + 1b per point)
  ...
  [
    "LAKE",                 // dataset name
    E,                      // names
    P,                      // polygons for each country (array of strings, 2b + 1b per point)
  ...
  [
    "STREAM",               // dataset name
    F,                      // names
    Q,                      // polygons for each country (array of strings, 2b + 1b per point)
  ...
  [                         
    ...
    A,                      // names
    L,                      // polygons for each country (array of strings, the first 2b represent the capital)
  ...
  [
    ...
    B,                      // names
    M,                      // polygons for each country (array of strings, the first 2b represent the capital)
    [...Array(50).keys()],  // questions not asked yet
    2,                      // how many bytes represent each item
    0,                      // fill color
    0,                      // stroke color
  ...
  [
    ...
    H,                      // names
    S,                      // polygons for each country (array of strings, the first 2b represent the capital)
  ...
  [
    ...
    I,                      // names
    T,                      // polygons for each country (array of strings, the first 2b represent the capital)
  ...
  [
    "PLACE",                // dataset name
    D,                      // names
    O,                      // polygons for each country (array of strings, the first 2b represent the capital)
...
O.start(i*.2);
O.frequency.setValueAtTime(415*1.06**(13-D.charCodeAt(i)),i*.2),
G.gain.setValueAtTime(.5, i*.2); 
G.gain.setTargetAtTime(.001, i*.2+.18,.005)
O.stop(i*.2+.19);
}*/
